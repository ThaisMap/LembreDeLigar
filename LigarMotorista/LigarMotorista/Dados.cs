...
namespace LigarMotorista
{
 ...
 Excel.XlSaveAsAccessMode.xlExclusive, misValue, misValue, misValue, misValue, misValue);
        ...
        public ObservableCollection<DevolucaoModel> LeDadosDevolucao<S, T>(string query) where S : IDbConnection, new()
                                           where T : IDbDataAdapter, IDisposable, new()
        {
            var ListaDeDevolucoes = new ObservableCollection<DevolucaoModel>();
            using (var conn = new S())
            {
                using (var da = new T())
                {
                    using (da.SelectCommand = conn.CreateCommand())
                    {
                        ...
                        foreach (DataRow row in ds.Tables[0].Rows)
                        {
                            aux = new DevolucaoModel()
                            {
                                Id = row["Id"].ToString(),
                                Nfd = row["Nfd"].ToString(),
                                Cliente = row["Cliente"].ToString(),
                                Fornecedor = row["Fornecedor"].ToString(),
                                Obs = row["Obs"].ToString(),
                                QuemLigou = row["QuemLigou"].ToString(),
                                NF = row["NF"].ToString()
                            };
                            ListaDeDevolucoes.Add(aux);
                        }
                    }
                }
            }

            return ListaDeDevolucoes;
        }

        public void NovaDevolucao(string idEntrega)
        {
            using (SQLiteConnection conn = new SQLiteConnection(conexao))
            {
                Motorista MotoDiaria = LeDadosEntrega<SQLiteConnection, SQLiteDataAdapter>("Select * from Entregas where Id=" + idEntrega)[0];
                ...
                using (SQLiteCommand cmd = new SQLiteCommand(conn))
                {
                    cmd.CommandText = "INSERT INTO Devolucoes (NF,Fornecedor,Cliente,Obs,QuemLigou) VALUES (@nf, @fornecedor, @cliente, @obs,@pessoa)";
                    cmd.Parameters.AddWithValue("@pessoa", MotoDiaria.QuemLigou);
                    cmd.Parameters.AddWithValue("@obs", MotoDiaria.Observacao);
                    cmd.Parameters.AddWithValue("@cliente", MotoDiaria.Cliente);
                    cmd.Parameters.AddWithValue("@fornecedor", MotoDiaria.Fornecedor);
                    cmd.Parameters.AddWithValue("@nf", MotoDiaria.NF);
